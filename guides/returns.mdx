...
sequenceDiagram
    ...
    loop Poll for Status Update
        YourSystem->>ShipBobAPI: GET /1.0/return (Check Status)
        ShipBobAPI-->>YourSystem: Return Status (e.g., Processing,Completed)
    end

    YourSystem->>ShipBobAPI: GET /1.0/return (Final Status and Details)
...
1. **ShipBob Sandbox Account**Sign up at [ShipBob's sandbox signup page](https://developer.shipbob.com/). If issues arise, note that you process over 1,000 shipments per month.
